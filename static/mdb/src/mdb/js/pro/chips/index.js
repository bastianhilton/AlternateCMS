...
class ChipsInput extends Chip {
  ...
  _handleDeleteEvents(event) {
    ...
    if (this.activeChip === null) {
      ...
      const index = this.chips.findIndex((chip) => chip === this.activeChip);
  ...
  _handleUpEvents(event) {
    ...
    if (this.numberClicks === this.chips.length + 1) this.numberClicks = 0;
  ...
  _handleDownEvents(event) {
    ...
    if (this.numberClicks <= 0) this.numberClicks = this.chips.length;
  ...
  _keyboardEvents(event) {
    ...
    if (target.value.length > 0 || this.chips.length === 0) return;
  ...
  _handleEditable() {
    ...
    this.chips.forEach((chip) => {
      EventHandler.on(chip, 'dblclick', (e) => {
        ...
        EventHandler.trigger(chip, EVENT_SELECT, { event: e, chips: this.chips });
      });
  ...
  _handleRemoveActiveClass() {
    this.chips.forEach((chip) => Manipulator.removeClass(chip, CLASSNAME_ACTIVE));
  }

  _handleAddActiveClass() {
    this.chips.forEach((chip) => Manipulator.addClass(chip, CLASSNAME_ACTIVE));
  }
  ...
  _handleTextContent() {
    ...
    this.chips.forEach((chip) => arr.push({ tag: chip.textContent.trim() }));
  ...
  _handleEvents(event, eventName) {
    ...
    const filterActive = this.chips.filter(
  ...
  _handleKeysInputToElement(event) {
    ...
    if (keyCode === ENTER) {
      ...
      this._handleRemoveActiveClass();
      this.numberClicks = this.chips.length + 1;
  ...
  _handleAddActiveClassWithKebyboard(num) {
    ...
    if (this.chips[num - 1] === undefined) {
      chip = this.chips[num - 2];
    } else {
      chip = this.chips[num - 1];
    }
