<template>
  <div>
    <form v-for="occassion in allOccassions" :key="occassion.id" method="POST" enctype="multipart/form-data" @submit.prevent>
      <nav class="navbar navbar-dark bg-dark">
        <div class="container-fluid">
          <a class="navbar-brand">
            <button type="reset" class="btn btn-warning">Reset</button></a>
          <a class="navbar-brand">
            <input type="submit" class="btn btn-warning" value="Save Occassion" @click="addOccassion" /></a>
        </div>
      </nav>
      <br>
      <div class="row">
                ...
                <table class="table">
                  <tbody>
                    <tr>
                      ...
                      <td>
                        <input v-model="code" type="text" required />{{ occassions.code }}
                      </td>
                    </tr>
                    <tr>
                      <td style="text-align: right;">Occassion Name</td>
                      <td>
                        <input v-model="name" type="text" />{{ occassions.name }}
                      </td>
                    </tr>
                    <tr class="col-lg-6">
                      <td>
                    ...
                    <tr class="col-lg-12">
                      <td>
                        <input v-model="description" type="textarea" value="Description" />{{ occassions.description }}
                      </td>
                    </tr>
                  </tbody>
                </table>
...
<script>
  ...
  import  occassions from "~/graphql/mutations/shop/occassions";
